...
namespace Aui {
    export namespace Navigation {
        ...
        export class Panel extends Aui.Panel {
            ...
            contexts: Aui.Navigation.Context[] = [];
            sorter: Aui.Navigation.Sorter;
             ...
             */
            getViewport(): Aui.Viewport {
                return this.getParent() as Aui.Viewport;
            }
             ...
             */
            addFolder(folder: Aui.Navigation.Context = null): void {
                new Aui.Window({
                    ...
                    buttons: [
                        new Aui.Button({
                            ...
                            handler: (button) => {
                                folder.remove();
                                (button.getParent() as Aui.Window).close();
                            },
                        }),
                        '->',
                        new Aui.Button({
                        ...
                        new Aui.Button({
                            ...
                            handler: async (button) => {
                                ...
                                if (isValid == true) {
                                    ...
                                    const $folders = Html.all(
                                        'div[data-component][data-role=context][data-context=FOLDER]'
                                    ).getList();
        ...
        export class Context extends Aui.Component {
            type: string = 'navigation';
            role: string = 'context';
            ...
            target: string;
            contextType: string;
            children: Aui.Navigation.Context[] = [];
            ...
            renderContent(): void {
                ...
                if (this.contextType == 'FOLDER') {
                    ...
                    $title.on('click', (e: MouseEvent) => {
                        if (this.getParent().$getComponent().hasClass('sorting') == true) {
                            (this.getParent() as Aui.Navigation.Panel).addFolder(this);
            ...
            remove(): void {
                if (this.contextType == 'FOLDER') {
                    ...
                    Html.all('> div[data-component]', $bottom).forEach(($child) => {
                        const child = Aui.getComponent($child.getAttr('data-component')) as Aui.Navigation.Context;
        ...
        export class Sorter {
            panel: Aui.Navigation.Panel;
            context: Aui.Navigation.Context = null;
            $context: Dom = null;
            $drag: Dom = null;
            $drop: Dom = null;
            tops: number[] = [];
             ...
             */
            constructor(panel: Aui.Navigation.Panel) {
                this.panel = panel;
                this.setEvent();
            }
                    ...
                    if ($button.getData('drag') == null) {
                        const drag = new Aui.Drag($button, {
                            ...
                            listeners: {
                                ...
                                drag: ($dom: Dom, tracker: Aui.Drag.Tracker) => {
                                    if (this.$drag == null) {
                                        this.$setDrag($dom.getParents('div[data-component][data-role=context]'));
                                    }
            ...
            getContexts(): any[] {
                ...
                Html.all('> div[data-component][data-role=context]', this.panel.$getContent()).forEach(
                    ($context: Dom) => {
                        ...
                        if ($context.getData('context') == 'FOLDER') {
                            ...
                            Html.all('div[data-component][data-role=context]', $children).forEach(($child: Dom) => {
            ...
            $setDrag($target: Dom): Dom {
                ...
                this.context = Aui.get($target.getData('component')) as Aui.Navigation.Context;
                ...
                Html.all('> div[data-component][data-role=context]', this.panel.$getContent()).forEach(
                    ($context: Dom) => {
                        ...
                        if ($context.getData('context') == 'FOLDER') {
                            ...
                            Html.all('div[data-component][data-role=context]', $children).forEach(($child: Dom) => {
                                const $link = Html.get(
                                    '> div[data-role=container] > div[data-role=content] > a',
                                    $child
                                );
                                let top = $link.getOffset().top - this.panel.$getContent().getOffset().top;
                                top += this.panel.getScroll().getPosition().y;
