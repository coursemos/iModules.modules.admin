            ...
            class Admin extends admin.Component {
                                                    ...
                                                    new Aui.Form.Field.RadioGroup({
                                                        ...
                                                        listeners: {
                                                            change: (field, value) => {
                                                                ...
                                                                const details = Aui.getComponent('details');
                                                                ...
                                                                const design = Aui.getComponent('design');
                ...
                administrators = {
                    ...
                    printScopesFieldSet: async (fieldset, readonly = false) => {
                        ...
                        for (const component of results.components) {
                            fieldset.append(new Aui.Form.FieldSet({
                                title: component.title,
                                items: (() => {
                                    const items = [];
                                    items.push(new Aui.Form.Field.Check({
                                    ...
                                    for (const scope of component.scopes) {
                                        items.push(new Aui.Form.Field.Container({
                                            label: scope.title,
                                            direction: 'column',
                                            items: (() => {
                                                const items = [];
                                                items.push(new Aui.Form.Field.Check({
                                                    boxLabel: this.printText('admin.administrators.lists.scope_all'),
                                                    name: scope.code,
                                                    readonly: readonly,
                                                    listeners: {
                                                        change: (field, checked) => {
                    ...
                    printGroupFieldSet: async (fieldset, is_ungrouped) => {
                        ...
                        if (groups.records.length > 0) {
                            fieldset.append(new Aui.Form.Field.CheckGroup({
                                name: 'group_ids',
                                columns: 1,
                                options: ((groups) => {
                                    let options = {};
                        ...
                        add: (group_id = null) => {
                            ...
                            new Aui.Window({
                                ...
                                listeners: {
                                    show: async (window) => {
                                        const form = window.getItemAt(0);
                                        const description = form.getItemAt(1);
                                        const fieldset = form.getItemAt(2);
                                        const scopes = await this.administrators.printScopesFieldSet(fieldset, is_component);
                                        if (scopes == true) {
                                            fieldset.enable();
                                        }
                                        if (group_id !== null) {
                ...
                database = {
                    tables: {
                         ...
                         */
                        delete: (grid, mode = 'truncate') => {
                            const selections = grid.getSelections();
                            if (selections.length == 0) {
                                return;
                            }
                            const tables = selections.map((selected) => {
                            ...
                            Aui.Message.delete({
                                ...
                                handler: async () => {
                                    if (mode == 'drop') {
                                        const tables = Aui.getComponent('tables');
